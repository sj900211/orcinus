...
export function loadMainWindow(
  ...
  observer?: MainWindowLoadObserver,
  opts?: { search?: string }
): void {
...
export function createMainWindow(
  ...
  // Why: 'workspace' windows are secondary app windows (project/satellite) — they must not
  // fight the main window over persisted bounds, tray behavior, or the primary trusted id.
  ...
  const state = installMainWindowStateLifecycle({
    mainWindow,
    persistWindowState: !isWorkspaceWindow,
  ...
  const focus = installMainWindowFocusLifecycle({
    ...
    mainWindow,
    opts,
    reloadMainWindow: (observer) => {
      if (isWorkspaceWindow) {
        // Why: keep the current URL so the orca-project boot params survive; loadMainWindow
        // would reboot this window as a second main-role document.
        mainWindow.webContents.reload()
        observer?.onLoaded?.()
        return
      }
      loadMainWindow(mainWindow, observer)
    },
  ...
  mainWindow.on('closed', () => {
    // Why: the dashboard pop-out is a companion of the main window only.
    if (!isWorkspaceWindow) {
