  ...
  for (const [, wc] of root.listeners) {
    if (!wc.isDestroyed()) {
      try {
        ...
        // Why (same class as the remote delivery guard): isDestroyed() stays false while a
    ...
    for (const [, wc] of root.listeners) {
      if (!wc.isDestroyed()) {
        try {
          ...
          // Why (same class as the remote delivery guard): the trailing-debounce timer can fire
