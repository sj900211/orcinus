...
// Why a registry, not imports: the SFTP pools live in ssh.ts, which imports this module — importing
// them back would be a cycle. Keyed so re-registration (macOS re-activation) stays idempotent.
const auxiliaryDrains = new Map<string, () => Promise<unknown>>()

export function registerSshShutdownAuxiliaryDrain(
  targetId: string,
  drain: () => Promise<unknown>
): void {
  auxiliaryDrains.set(targetId, drain)
}
...
function sshShutdownTasks(targetIds: readonly string[]): SshShutdownTask[] {
  return [
    ...
    ...[...auxiliaryDrains].map(([targetId, drain]) => ({ targetId, promise: drain() }))
  ]
}
