...
describe('renderer startup runtime routing', () => {
  ...
  it('checkpoints activeView and all session snapshots through one beforeunload handler (#9002)', () => {
    ...
    // runs the exact serialize+stage path without arming the quit-flavored degrade tiers.
    const stageCheckpointStart = source.indexOf('export function stageWorkspaceSessionCheckpoint(')
    const stageCheckpointEnd = source.indexOf(
      'type RemoteWorkspaceUploadAuthority = {',
      stageCheckpointStart
    )
    expect(stageCheckpointStart).toBeGreaterThanOrEqual(0)
    expect(stageCheckpointEnd).toBeGreaterThan(stageCheckpointStart)
    const stageCheckpointBlock = source.slice(stageCheckpointStart, stageCheckpointEnd)

    expect(stageCheckpointBlock).toContain(
    ...
    expect(stageCheckpointBlock).toContain('window.api.app.stageBeforeUnloadSync({')
    expect(stageCheckpointBlock).toContain('sessions: sessionSnapshots')
    expect(stageCheckpointBlock).toContain('ui: buildActiveViewUnloadPatch(freshState)')
    expect(stageCheckpointBlock).toContain('!isIntentionalAppRestartInProgress()')
    expect(stageCheckpointBlock).toContain('freshState.openFiles.some((file) => file.isDirty)')
    expect(stageCheckpointBlock).toContain('sessions: []')
    // Line-ending normalized so a CRLF Windows checkout matches the same structure.
    expect(stageCheckpointBlock.replace(/\r\n/g, '\n')).toContain(
    ...
    // Why: a closing project window hands its session slice to main inside the same guarded quit checkpoint.
    expect(checkpointBlock).toContain('stageProjectWindowSessionHandback()')
