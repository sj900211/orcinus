...
export function renderSftpSettingsSection(
  ...
  return model.showDesktopOnlySettings ? (
    <SettingsSection
      id="sftp"
      title={translate('auto.components.settings.Settings.sftpHostsTitle', 'SFTP Hosts')}
      description={translate(
        'auto.components.settings.Settings.sftpHostsDescription',
        'Browse and transfer files on remote servers over SFTP.'
      )}
      searchEntries={navigation.getSectionSearchEntries('sftp')}
    >
      {view.isSectionMounted('sftp') ? <RemoteHostsPane /> : null}
    </SettingsSection>
  ) : null
}
